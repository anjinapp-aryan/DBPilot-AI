...
def test_build_provider_configs_reads_deepseek_nvidia_settings() -> None:
    settings = Settings(
        deepseek_nvidia_api_key="nvapi-test",
    ...
    assert configs["deepseek"].api_key == "nvapi-test"
...
def test_deepseek_nvidia_api_key_reads_deep_sheek_alias(monkeypatch) -> None:
    monkeypatch.setenv("DEEP_SHEEK_NVIDIA_API_KEY", "nvapi-alias-test")
    settings = Settings()
    assert settings.deepseek_nvidia_api_key == "nvapi-alias-test"
...
def test_build_provider_configs_reads_groq_settings() -> None:
    settings = Settings(groq_api_key="gsk-test", groq_model="llama-3.3-70b-versatile")
    configs = build_provider_configs(settings)
    assert configs["groq"].api_key == "gsk-test"
